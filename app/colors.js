...
function startTime (key) {
  var time = times[key] || (times[key] = {n: 0, t: 0, p: 0})
  time.s = performance.now()
}
function pauseTime (key) {
  var time = times[key]
  time.p += performance.now() - time.s
  time.s = 0
}
function stopTime (key) {
  var time = times[key]
  var t = (time.s ? performance.now() - time.s : 0) + time.p
  time.t += (t - time.t) * (1 / ++time.n)
  time.p = 0
}
